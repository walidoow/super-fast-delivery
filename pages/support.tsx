...
export default function Support() {
  ...
  const [currentCategory, setCurrentCategory] = useState(Category.MANAGE_ACCOUNT);
  ...
  return (
    <div className="h-full bg-primary-yellow-green flex flex-row">
      <ScrollArea className="h-full w-full">
        <div className={"h-full w-full flex flex-col justify-center place-items-center py-8"}>
          <h2 className="scroll-m-20 pb-2 text-4xl font-semibold tracking-tight first:mt-0 space-y-1">
            Customer Service
          </h2>
          <Separator className="w-16 h-[2px] my-5 bg-black"></Separator>
        </div>
        <OpenCases/>
        ...
        <div className={"min-h-[80vh] flex flex-row"}>
          ...
          <div className={"flex-grow py-8 px-96"}>
            {currentCategory == Category.MANAGE_ACCOUNT ? <ManageAccount/> : undefined}
            {currentCategory == Category.OPEN_CS_SESSION ? <OpenCSSession/> : undefined}
            {currentCategory == Category.RETURN_AND_REFUNDS ? <ReturnRefundOrder/> : undefined}
            {currentCategory == Category.CANCEL_ORDER ? <CancelOrder/> : undefined}
          </div>
        </div>
      </ScrollArea>
    </div>
