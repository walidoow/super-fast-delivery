  ...
  return (
    <div className="mx-12">
      <div className="my-4">
        ...
        <p className="text-sm text-muted-foreground">Cancel a placed order</p>
      </div>
      <Separator className="my-6" />
      <div className="mx-6">
        <p className="leading-7 [&:not(:first-child)]:mt-6 my-3">
          Placed orders
        </p>
          ...
          {cards.map((card, index) => (
            <Card className="w-[270px]">
              ...
              <CardFooter className="flex justify-center">
                <Button onClick={() => handleDeleteCard(index)}>Cancel</Button>
              </CardFooter>
            </Card>
          ))}
        </div>
        <p className="text-sm text-muted-foreground my-3">
          Orders can only be cancelled for up to 24 hours after placing the
          order. If such is the case, a full refund will be provided. Otherwise,
          see Returns and Refunds.
        </p>
      </div>
    </div>
  );
